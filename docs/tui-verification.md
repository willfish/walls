...
- Destructive confirmations: with a current wallpaper, press `d` and confirm the prompt names the target; unrelated keys do not perform unrelated actions, Esc cancels, and a second `d` confirms. Shift+X provider reset still requires a second Shift+X, reports queue/cache/download/history/current counts, and Esc cancels.
